...
//uncomment main and referecnes to crypto to test locally
// and use tsconfig.local.json
...
const getCoinbasePrice = (base: string, quote: string, date: Date) => {
  const formattedDateToday = Utilities.formatDate(
    new Date(),
    "GMT",
    "yyyy-MM-dd"
  );
...
const getBinancePrice = (symbol: string, quote: string, date: Date) => {
  ...
  if (formattedDate !== formattedDateToday) {
    //it's not today
    console.log(
      `Fetching kline price because it's not today: ${formattedDate} != ${formattedDateToday}`
    );
    ...
    if (val === "event: balance") {
      ...
      if (jsonString) {
        ...
        const allAddressTotal = ethAddresses.reduce((acc, ethAddress) => {
          let addressRootData = json.balances?.[ethAddress];

          // within here, we should be subtracting assets with "hide=true" like value of like KICK
          // addressRootData?.products?.assets

          let metaArray = addressRootData?.meta;
          if (metaArray) {
...
const getByBitBalances = async (apiKey: string, secret: string) => {
  ...
  let requestUrl = `https://alertzmanager.io/api/proxy?${queryString}&host=${"api.bybit.com"}&path=${"/contract/v3/private/account/wallet/balance"}`;
  ...
  if (isGoogle()) {
    ...
    try {
      ...
      //if (response.getResponseCode() !== 200) {
      log(
          "error " + response.getResponseCode() + response.getContentText()
      );
      //}
...
const createByBitSignature = (parameters, secret) => {
  ...
  log("ordered: " + JSON.stringify(orderedParams));

  const signature = isGoogle()
      ? Utilities.computeHmacSha256Signature(orderedParams, secret)
          .map(function (chr) {
            return (chr + 256).toString(16).slice(-2);
          })
          .join("")
      : "" //createHmac("sha256", secret).update(orderedParams).digest("hex");
...
//   getByBitBalances(process.env.BYBIT_API_KEY, process.env.BYBIT_API_SECRET);
// };
